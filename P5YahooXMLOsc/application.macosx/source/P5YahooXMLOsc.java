...
public class P5YahooXMLOsc extends PApplet {
...
public void draw() {
    ...
    if (firsttime == 1 || timenow - oldtime >= 600000) {
      ...
      if (myval == null) {
        ...
        try {
          ...
          if (sunits.equals("km/h") ) speedmph = speed / 1.60934400061f;
          ...
          _windSpeed = speedmph*1.60934400061f; //Convert to km/h
  ...
  if (frameCount % 300 == 1 && frameCount > 10) {
    ...
    wd.add( PApplet.parseInt(_windDirection) ); 
    oscP5.send(wd, sendAddress);

    OscMessage ws = new OscMessage("/windSpeed");
    ws.add( PApplet.parseInt(_windSpeed) ); 
    ...
    gwc.add( PApplet.parseInt(_groupedWeatherCode) ); 
...
public float ftoc(float f) {
  f = (f-32) * 5/9;
  f = PApplet.parseInt(f *10 + 0.5f);
  return (f/10);
}

public void Cprintme(String temp) {
  float mywidth = textWidth(temp);
  text(temp, (width - mywidth) / 2, ystep);
  ystep = ystep + linespace;
}

public void Lprintme(String temp, int margin) {
...
public void showWind(float speed, int direction) {
  float rcp = PApplet.parseFloat(direction + 180 ) % 360;
  noFill();
  float w = textWidth("W");
  int xc = width - 150;
  int yc = ystep + 21;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "P5YahooXMLOsc" };
    if (passedArgs != null) {
